...
online_users = {
    "user1": {
        "name": "tr1x_em",
        "status": "Online",
        "avatar": "https://i.ibb.co/L1svvRg/tr1x-em.png",
    },
}
...
def register_page():
    ...
    with ui.column().classes("absolute top-0 right-0 p-4"):
        with ui.element().classes("max-[420px]:hidden top-0 right"):
            ...
            ).props("flat fab-mini color=black").bind_visibility_from(
    ...
    with ui.column().classes("absolute-center items-center"):
        ...
        name_input = (
            ui.input("Name").props("rounded outlined").classes("w-full max-w-xs")
        )
        username_input = (
            ui.input("Username").props("rounded outlined").classes("w-full max-w-xs")
        )
        password_input = (
        ...
        option_input = (
            ui.select(label="Avatar", options=["Robots", "Monster", "Cat", "Human"])
            .props("rounded outlined")
            .classes("w-full max-w-xs")
        )
...
def main_page() -> None:
    ...
    with ui.column().classes("absolute-center items-center"):
        ui.image(
            "https://static.vecteezy.com/system/resources/thumbnails/000/602/787/small/83038926.jpg"
        ).classes("rounded-full w-48 h-48 ml-4")
        ui.label("Welcome to Zwitter!!").classes("text-6xl")
        with ui.row().classes("items-center").classes("text-2xl"):
            ui.button("Log in", icon="login", on_click=lambda: ui.navigate.to("/login"))
            ui.button(
                "Register", icon="home", on_click=lambda: ui.navigate.to("/register")
            )

    with ui.column().classes("absolute bottom-0 right-0 p-4"):
        with ui.row():
            ui.html("Made with ❤️ by")
            ui.link("tr1x_em", "https://trix.is-a.dev").classes(
        ...
        for user_id, user_info in online_users.items():
            with ui.item():
                with ui.item_section().props("avatar").style("position: relative;"):
                    ...
                    ui.html(
                        '<span style="position: absolute; bottom: 2px; right: 20px; '
        ...
        for user_id, avatar, text, stamp in messages:
            ...
            with ui.row().classes("w-full items-center chat-message-container "):
                if own_id == user_id:
                    ...
                    with ui.button(
                        icon="reply",
                        on_click=lambda msg_text=text, sender_name=user_id: send_reply(
                            msg_text, sender_name
                        ),
                    ).props("flat").classes("reply-button rounded-full mr-1"):
                    ...
                    ).props("flat").classes("reply-button rounded-full ml-2"):
                        ui.label("").classes("hidden")
    ...
    if not checkuser(username):
        ...
        with ui.footer().classes("bg-black"):
            with ui.column().classes("w-full max-w-3xl mx-auto my-6"):
                with ui.row().classes("w-full no-wrap items-center"):
                    ...
                    ui.button(icon="send", on_click=send).classes(
                        "ml-3 flex items-center"
                    ).props("rounded")

        ui.add_css(
            r'a:link, a:visited {color: inherit !important; text-decoration: none; font-weight: 500; } {background-image: url("https://i.imgur.com/Gk3qlQ2.png"); background-size: cover; background-position: center; background-repeat: no-repeat;}'
        )
        ...
        with ui.column().classes("w-full max-w-2xl mx-auto items-stretch"):
            with ui.column().style(
                "position: absolute; top: 0; left: 0; height: 80%; width: 30%; z-index: 1; padding: 10px;"
            ):
                update_user()
